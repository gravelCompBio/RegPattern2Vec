...
#         print('self: {}'.format(sys.getsizeof(self)))
    ...
    def walk_progress(self,source_node_types, target_node_types, initials, all_nw = 40, all_wl = 40, num_source = 0
        ...
        walk_file = '{}_{}_{}_{}_{}_.csv'.format(walk_file_prefix, self.all_nw, self.all_wl, num_source, num_allow_loop)
        if self.postfix != '':
            walk_file = '{}_{}_{}_{}_{}_{}.csv'.format(walk_file_prefix, self.all_nw, self.all_wl, num_source, num_allow_loop, self.postfix)
            
        manager = mp.Manager()
        job_number = mp.cpu_count()
        
        q = manager.Queue()    
        pool = mp.Pool(job_number)

        #put listener to work first
        watcher = pool.apply_async(self.write_walks, (walk_file, q))
        ...
        for i in range(len(source_node_lists)):
            job = pool.apply_async(self.walk, (source_node_lists[i], q))
            jobs.append(job)
...
#                                     self.all_wl, num_source = num_source, num_allow_loop=num_allow_loop, walk_file_prefix=walk_file_prefix)
    ...
    def write_walks(self, walk_file, q):
        with open(self.data_path + walk_file, 'w') as f:
            while 1:
                m = q.get()
                if m == 'kill':
                    f.write('killed')
...
#         del(scores[p_index]) # newly added 
    ...
    def walk(self, selected_nodes, q):
