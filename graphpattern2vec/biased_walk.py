...
class BiasedWalk(object):
    ...
    def __init__(self, graph_pattern, loop_index, type_loop,  data_path = 'data/',  train_file = 'df_train.csv', postfix = '', degree_biased = True):
...
#         del(scores[p_index]) # newly added 
    ...
    def walk(self, graph_pattern, loop_index, type_loop, initials, number_of_walks, 
             walk_length, num_source = 1000, num_allow_loop = 10, walk_file_prefix = 'walks_nx'):
        source_nodes = self.get_nodes_by_types(graph_pattern[0].keys())
        print('source nodes: {}'.format(len(source_nodes)))
        ...
        outfile = open(self.data_path + walk_file, 'w')
