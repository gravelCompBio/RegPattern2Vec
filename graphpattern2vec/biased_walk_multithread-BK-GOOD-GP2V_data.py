...
#                 for results in p.imap_unordered(mp_worker, data_list):
#                     for result in results:                        
#                         writer.write(result)
                
        #####
        for i, s in enumerate(source_node_lists):
            j = multiprocessing.Process(target=self.walk, args=(walk_file, s, q))
            jobs.append(j)
        for j in jobs:
            j.start()
...
#         del(scores[p_index]) # newly added 
    ...
    def walk(self, walk_file, selected_nodes,q):
